...
function BookPageContent({ resolvedOwnerUid }: BookPageContentProps = {}) {
  ...
  const computeSlots = (forServiceId?: number | string): Array<{ time: string; available: boolean; reason?: string }> => {
    ...
    // Helper: detect "Any Staff" staffId values (null, undefined, "", "null", "any", etc.)
    ...
    const isSlotBlockedByCurrentSelection = (slotStartMin: number): { blocked: boolean; reason?: string } => {
      ...
      if (isAnyStaffSelected) {
        ...
        for (const otherServiceId of bkSelectedServices) {
          ...
          const otherIsAnyStaff = isAnyStaffValue(otherStaffId);
    ...
    for (let slotStartMinutes = startMinutes; slotStartMinutes < endMinutes; slotStartMinutes += interval) {
      ...
      // For "Any Staff" mode, pass null as staffId and eligible staff IDs so holds
      // are only blocking when ALL eligible staff are consumed by other sessions' holds.
      const isHeld = isSlotHeldByOther(
        slotHolds,
        isAnyStaffSelected ? null : (staffIdForService || null),
        timeStr,
        serviceDuration,
        sessionId,
